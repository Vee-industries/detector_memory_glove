...
{\LARGE\bfseries An iota of signal in a flood of noise: a stateful detector \mbox{improves} synthetic intrusion detection from 13\% to 92\%\par}
\vspace{6pt}
...
\noindent\textit{What predates the sprint.} Hugging Face disclosed the incident on 16 July 2026. I began this work on 24 August in response to it. All experiments and the written answers this report is assembled from were completed between 24 August and 9 September, and the file timestamps in the repository record that sequence. The sprint window was used for a verification pass, in which the test suite was re-run and every table in this report was reconciled against its source output file, and for final assembly.
...
\item Detector: logistic regression on [current event, memory]. Memory $s_t=\alpha s_{t-1}+(1-\alpha)x_{t-1}$, $\alpha=e^{-1/\tau}$, scored with $s_{t-1}$ so the current event never sees itself. Fast $\tau=120$ events, slow $\tau=2500$. Thresholds set on a calibration set so that 5\% of benign review windows trigger an alert, a benign window being any window of a benign entity or any pre-onset window of an attacker; the union's per-tier budgets tuned the same way.
...
The short history sweep catches 20\% of attacks within 15 events, against 10\% for the myopic scorer on the same synthetic data. On the long campaign the hit rate is 52\% within 24 hours and 92\% within 120 hours, against 13\% for the same scorer without memory (Table~\ref{tab:fair}, Figure~\ref{fig:long}). Table~\ref{tab:fair}'s memory tier is trained on short campaigns, as in the original design; every experiment from Table~\ref{tab:controls} onward trains the same tier on the long-campaign mix, which reads three points higher (55\% and 94\%). The lower figure is the headline. Bootstrap 95\% intervals for the long-mix tier over the 9,994 test attackers that reached onset, thresholds held at their calibration values: memory 94\% [94, 94] against no memory 13\% [13, 14] at 120 hours, a difference of 81\% [80, 82]; at 24 hours the difference is 44\% [43, 45]. CUSUM run on the no-memory score, with its reference level and slack set on the calibration set, lands within two points of the memory layer at both windows (Table~\ref{tab:fair}); the slack chosen was zero, because subtracting the mean benign score already removes the drift and any positive slack starves the statistic of the weak per-event signal it is meant to accumulate; the two carry the same information, and the memory layer is the one learned from data. Figure~\ref{fig:roc} gives detection against false-alarm rate for the no-memory scorer, CUSUM, and the memory layer, so the 5\% used throughout is one point on a curve rather than a choice that favours one policy.
...
\bottomrule
\end{tabular}
\caption{Fair setup. Detection within the review window after onset, at 5\% false alarms. The memory row is the short-campaign-trained tier, as in the original design; the CUSUM row is trained on the long-campaign mix. Trained on that same mix, the memory tier reads 0.547 and 0.940 against CUSUM's 0.526 and 0.919.}\label{tab:fair}
\end{table}
...
\midrule
Unbroken (long-mix-trained tier) & 0.10 / 0.55 & 0.13 / 0.94 \\
...
\caption{Left: detection against per-event signal. Middle: memory's advantage against $\hat R_{\rm det}$, flat until 0.6 then steep; the 24-hour curve turns down at the top. Right: $\hat R_{\rm det}$ against signal, monotone.}\label{fig:sweep}
\end{figure}
...
\noindent\textit{Does publishing the age finding help attackers?} An attacker who learns that the memory carries an age term can rotate fresh entities; the incident's agent already did. The mitigations are in the artifact: the age-term-removed variant with its minimum-history gate, and keying memory on identities that outlive the pod (Table~\ref{tab:keys}). The published thresholds come from a synthetic generator, and each instance of the detector is calibrated to the infrastructure it runs on.
...
I have a disability affecting executive function, measured at the fourth percentile. Composition and sequencing are the parts I cannot do unaided, and I use AI as an accommodation for them. The division of labour below is exact.

\noindent\textit{Mine.} The research question, the structural-remainder framing, the matched-false-positive design, the tiered trigger, the decision not to automate mitigation, and the judgement at every point about what to run next. This report is written from my answers to a structured interview, kept verbatim in the repository. The lagged-feature control I had listed as the next step showed the original result was an entity-age artifact, and I directed the corrected protocol that replaced it.

\noindent\textit{Claude's, under my direction.} Implementing the code and experiments I specified, typesetting this report from my written answers, editing those answers to my written voice specification, and carrying the organising and sequencing load. Every number was checked against the output files and every statement about the incident against the cited sources. The full disclosure is in the repository.
