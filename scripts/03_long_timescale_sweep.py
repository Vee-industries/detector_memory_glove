...
def apply_state(features, tau):
    """
    Standard EMA state, same as locked glove.
    """
    T, d = features.shape
...
def onset_window(campaign, window_length):
    """
    Returns first risk=1 index and the end index of the onset window,
    clipped to campaign length.
    """
    risk = campaign["risk"]
...
def benign_windows(campaign, window_length, stride):
    """
    Yields benign windows with the given length and stride.
    Only windows fully contained before risk onset.
    """
    risk = campaign["risk"]
    if not np.any(risk == 1):
        return []
...
def main():
    ...
    # 24h = 68.6 steps, 36h = 102.9, 72h = 205.7
    stride = 34
    window_lengths = [68, 103, 206]  # 24h, 36h, 72h
    ...
    tau_fast = 120
    tau_slow_candidates = [500, 800, 1200, 1600, 2000, 2500, 3000]
    ...
    print("\nStarting time-based tau and window sweep...")
    ...
    with open("outputs/fast_slow_timebased_results.csv", "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow([
    ...
    print("\nSaved fast_slow_timebased_results.csv")
    print("Run complete.")
